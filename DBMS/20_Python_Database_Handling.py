...
conn = mysql.connector.connect(user='kamesh', password='12345678',
                              host='db4free.net', database = 'dbms_test')
#, database = 'forsk_test'
...
client = pymongo.MongoClient("mongodb+srv://admin:FZMv8d5qWCSjb8z@cluster0-4fycb.mongodb.net/test?retryWrites=true&w=majority")
mydb = client.test


def add_employee(idd, first, last, pay):
    unique_employee = mydb.test_coll.find_one({"id":idd})
    if unique_employee:
        return "Employee already exists"
    else:
        mydb.test_coll.insert_one(
                {
                "id" : idd,
                "first" : first,
                "last" : last,
